...
fn main() {
    const X_SCALE: u32 = 20;
    const Y_SCALE: u32 = 20;

    let sdl_context = sdl2::init().unwrap();
    let video_subsystem = sdl_context.video().unwrap();

    let window = video_subsystem
        .window("RustES", 32 * X_SCALE, 32 * Y_SCALE)
        .position_centered()
        .build()
        .unwrap();
    ...
    canvas.set_scale(X_SCALE as f32, Y_SCALE as f32).unwrap();
    ...
    let mut scree_state = [0 as u8; 32 * 3 * 32];
