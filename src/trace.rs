...
pub fn trace(cpu: &CPU) -> String {
    ...
    let (mem_addr, stored_value) = match op.mode {
        ...
        _ => {
            let addr = cpu.get_absolute_address(&op.mode, program_counter + 1);
    ...
    let tmp = match op.len {
        ...
        3 => {
            let addr_low = cpu.mem_read(program_counter + 1);
            let addr_high = cpu.mem_read(program_counter + 2);
            hex_dump.push(addr_low);
            hex_dump.push(addr_high);
    ...
    let asm_str = format!(
        "{:04x}  {:8} {: >4} {}",
        program_counter, hex_str, op.mnemonic, tmp
    )
    .trim()
    .to_string();
